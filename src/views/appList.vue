...
<div id="table_app" >
    <div class="wrapper">
        ...
        <section class="sec" v-bind:class="{styleSec: openAside}">
                                    ...
                                    <table id="table" class="table table-striped table-hover cursor-pointer dataTable no-footer" role="grid" aria-describedby="table_info" style="width: 1291px;">
                                        <thead>
                                            <tr role="row">
                                                ...
                                                <th class="sorting" v-bind:class="{ sorting_desc: sortStatus }" tabindex="0" aria-controls="table" rowspan="1" colspan="1" aria-label="Status: activate to sort column ascending" style="width: 103px;" v-on:click="sortByStatus">Status</th>
                                        ...
                                        <tbody>
                                            ...
                                            v-for="item in paginate" >
                                                ...
                                                <td class="dataTables_empty">
                                                    <label v-bind:for="item.uuid" class="c-checkbox">
                                                        <input type="checkbox" v-bind:id="item.uuid" v-bind:value="item.uuid"   :checked="item.suspended != 'true'" v-on:click="checkedStatus(item.uuid,item.suspended)"> 
...
var devCount = localStorage.getItem("devCount");
    ...
    created: function(){
           ...
           document.title = 'APP List :: Developer UI';
        },
  computed: {
    totalPages: function() {
            if(this.searchNname == ''){
       ...
       getUserInfo: function(){
          ...
          getData: function(){
              ...
              .then(response => {
                  ...
                  this.leftAppCount= response.body.ret.length;
                  console.log(response)
                    if(this.app_count == 0){
                        this.countStart = 0;
